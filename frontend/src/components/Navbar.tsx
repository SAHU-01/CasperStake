...
//     { href: "/institutional", label: "Institutional", icon: "🏛️", badge: "ZK" },
//   ];
        ...
        <nav className="hidden lg:flex items-center gap-1">
          {navLinks.map((link) => (
            ...
            >
              ...
              {link.label}
              {link.badge && (
                <span className="bg-purple-500/20 text-purple-400 text-[9px] font-bold px-1.5 py-0.5 rounded">
                  {link.badge}
                </span>
              )}
            </Link>
        ...
        <div className="flex items-center gap-3">
          {connected ? (
            <div className="flex items-center gap-3">
              ...
              <button
                onClick={disconnect}
                className="px-4 py-2 bg-red-600/20 border border-red-600 text-red-500 font-bold text-sm hover:bg-red-600 hover:text-white transition rounded-lg"
              >
                Disconnect
              </button>
            </div>
            ...
            <button
              onClick={connect}
              disabled={loading}
              className="px-4 py-2 bg-[#CDFF00] text-black font-bold text-sm hover:bg-[#b8e600] disabled:opacity-50 rounded-lg"
            >
              {loading ? "Connecting..." : "Connect Wallet"}
            </button>
          )}
          ...
          <div className="grid grid-cols-2 gap-2">
            {navLinks.map((link) => (
              ...
              >
                ...
                {link.label}
                {link.badge && (
                  <span className="bg-purple-500/20 text-purple-400 text-[9px] px-1.5 py-0.5 rounded ml-auto">
                    {link.badge}
                  </span>
                )}
              </Link>
